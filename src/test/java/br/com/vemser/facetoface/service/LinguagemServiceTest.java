...
public class LinguagemServiceTest {
    ...
    public void testarBuscarLinguagemPeloNomeComSucesso(){
        ...
        //Act
        when(linguagemRepository.findByNome(NOME)).thenReturn(Optional.of(linguagemEntity));
    ...
    @Test
    public void tentarBuscarLinguagemPeloId() throws RegraDeNegocioException {
        // SETUP
        final int id = 1;
        final String nome = "Java";
        LinguagemEntity linguagemEsperada = new LinguagemEntity();
        linguagemEsperada.setIdLinguagem(id);
        linguagemEsperada.setNome(nome);

        //Act
        when(linguagemRepository.findById(id)).thenReturn(Optional.of(linguagemEsperada));
        LinguagemEntity linguagem = linguagemService.findById(id);

        //Assert
        assertEquals(id, linguagem.getIdLinguagem());
    ...
    @Test
    public void deveTestarCriacaoComSucesso() throws RegraDeNegocioException{
        // Setup
        LinguagemDTO linguagemDTO = getLinguagemDTO();
        LinguagemEntity linguagem = getLinguagemEntity();
        ...
        // Act
        when(linguagemService.create(linguagemDTO)).thenReturn(linguagem);
    ...
    public void testarCriarLinguagemCasoNaoExistaUmaComSucesso(){
        ...
        //Act
        when(linguagemRepository.findByNome(nome)).thenReturn(Optional.empty());
        when(linguagemRepository.save(any())).thenReturn(linguagem);
